...
def create_chain_csp(n):
    ...
    for i in range(1, n):
        ...
        csp.add_binary_factor(var1, var2, xor_function)
    # ### END CODE HERE ###
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            csp.add_binary_factor(f'Q{i}', f'Q{j}', lambda x, y: x != y and abs(x - y) != abs(i - j))
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            most_constrained_variable_index = float('inf')

            for i, var in enumerate(self.csp.variables):
                if var not in assignment:
                    consistent_values = sum(
                        1 for value in self.domains[var] if self.get_delta_weight(assignment, var, value) != 0)

                    if consistent_values < min_domain_size or (
                            consistent_values == min_domain_size and i < most_constrained_variable_index):
                        most_constrained_variable = var
                        most_constrained_variable_index = i
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    if len(variables) == 0:
        ...
        for i, Xi in enumerate(variables):
            incremental_sum = ('sum', name, Xi)
            if i == 0:
                domains = [(0, val, 0+val) for val in csp.values[Xi]]
                csp.add_variable(incremental_sum, domains)
            else:
                domains = list(
                    set(
                        [
                            (val1[2], val2, val1[2] + val2) for val1 in csp.values[previous_incremental_sum]
                            for val2 in csp.values[Xi] if val2 + val1[2] <= maxSum
                        ]
                    )
                )
                csp.add_variable(incremental_sum, domains)
                csp.add_binary_factor(previous_incremental_sum, incremental_sum, lambda previous_incremental_sum, incremental_sum: previous_incremental_sum[2] == incremental_sum[0])
            csp.add_binary_factor(Xi, incremental_sum, lambda Xi, incremental_sum: Xi == incremental_sum[1])
            previous_incremental_sum = incremental_sum
    newDomain = list(set(val[2] for val in domains))
    csp.add_variable(result, newDomain)
    csp.add_binary_factor(result, incremental_sum, lambda result, incremental_sum: result == incremental_sum[2])
    return result
    # ### END CODE HERE ###
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    domains = [
                        i for i in range(self.bulletin.courses[cid].minUnits, self.bulletin.courses[cid].maxUnits+1)
                    ] + [0]
                    csp.add_variable((cid, quarter), domains)
                    csp.add_binary_factor(
                        (request, quarter), (cid, quarter),
                        lambda course, unit: unit == 0 if course != cid else unit != 0
                    )
                    requests.append((cid, quarter))
            sum_variable = get_sum_variable(csp, 'Total units in' + quarter, requests, self.profile.maxUnits)
            csp.add_unary_factor(
                sum_variable,
                lambda sum_variable: self.profile.maxUnits >= sum_variable >= self.profile.minUnits
            )
        # ### END CODE HERE ###
